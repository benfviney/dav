...
export class Basic extends Transport {
  ...
  send(request, url, options) {
    return co(function *() {
      ...
      let auth = "Basic " + encode(this.credentials.username + ":" + this.credentials.password);
  ...
  send(request, url, options={}) {
    return co(function *() {
      ...
      try {
        ...
        if (sandbox) sandbox.add(xhr);
        xhr.open(request.method, this.proxyUrl + url, true /* async */);
        ...
        if (options.retry && xhr.status === 401) {
          ...
          // Retry once at most.
          options.retry = false;
          return this.send(request, this.proxyUrl + url, options);
        }
